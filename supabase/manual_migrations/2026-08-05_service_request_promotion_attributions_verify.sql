-- Read-only verification after applying 2026-08-05_service_request_promotion_attributions.sql
...
WHERE c.table_schema = 'public'
  AND c.table_name = 'service_request_promotion_attributions'
...
WHERE nsp.nspname = 'public'
  AND rel.relname = 'service_request_promotion_attributions'
  AND con.contype IN ('p', 'u')
ORDER BY con.conname;

-- Expected: PK, UNIQUE(attribution_token)
...
-- =============================================================================
-- 5) Foreign keys (expect only service_request_promotions, ON DELETE CASCADE)
-- =============================================================================
SELECT
  con.conname AS constraint_name,
  pg_get_constraintdef(con.oid) AS definition
...
-- Expected: single FK to public.service_request_promotions(id) ON DELETE CASCADE
...
WHERE n.nspname = 'public'
  AND c.relname = 'service_request_promotion_attributions';
...
WHERE nsp.nspname = 'public'
  AND cls.relname = 'service_request_promotion_attributions'
...
WHERE n.nspname = 'public'
  AND t.relname = 'service_request_promotion_attributions'
ORDER BY i.relname;

-- Expected: PK, UNIQUE(attribution_token),
-- idx_service_request_promotion_attributions_promotion_first_seen,
-- idx_service_request_promotion_attributions_utm_source_first_seen

-- =============================================================================
-- 10) Table and column comments
-- =============================================================================
SELECT obj_description(
  format('%I.%I', 'public', 'service_request_promotion_attributions')::regclass
) AS table_comment;
...
WHERE cols.table_schema = 'public'
  AND cols.table_name = 'service_request_promotion_attributions'
...
-- =============================================================================
-- 11) Forbidden columns must not exist (PII / auth / payment / signup)
-- =============================================================================
SELECT column_name
...
-- =============================================================================
-- 12) Informational counts (do not prove immutability of other tables)
-- =============================================================================
