    ...
    <article className="overflow-hidden rounded-2xl border border-black/5 bg-white shadow-md transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <button
          ...
          className="group absolute right-3 top-3 inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow-sm ring-1 ring-black/10 transition hover:bg-white"
          aria-label={saved ? "Unsave specialist" : "Save specialist"}
          title={saved ? "Видалити зі збережених" : "Зберегти спеціаліста"}
        >
          <svg
            viewBox="0 0 24 24"
            className={`h-5 w-5 ${saved ? "fill-rose-500 text-rose-500" : "fill-transparent text-gray-700"}`}
            stroke="currentColor"
            strokeWidth="1.8"
          >
      ...
      <div className="space-y-4 p-5">
        <div>
          ...
          {specializationText ? (
            <p className="mt-1 line-clamp-1 text-sm text-gray-600">{specializationText}</p>
          ) : null}
        </div>
