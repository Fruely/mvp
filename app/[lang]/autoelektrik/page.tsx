...
import { autoelektrikContent } from "@/content/seo/v2/autoelektrik.content";

export const dynamic = "force-dynamic";

const DOMAIN = process.env.APP_URL || "https://freuly.de";
const SLUG = autoelektrikContent.slug;

export async function generateMetadata({ params }: { params: { lang: string } }) {
  const lang = isSupportedLang(params.lang) ? (params.lang as Lang) : "de";
  const c = autoelektrikContent.content[lang];
  return {
    title: c.metaTitle,
...
export default async function AutoelektrikPage({
  params,
}: {
  params: { lang: string };
}) {
  ...
  const { data } = await supabase
    ...
    .or(autoelektrikContent.filterOr ?? "")
    .limit(12);

  return (
    <CategoryContentPage
      lang={lang}
      content={autoelektrikContent.content[lang]}
      specialists={data ?? []}
    />
  );
}
