...
import { autoMobilitaetContent } from "@/content/seo/v2/auto-mobilitaet.content";

export const dynamic = "force-dynamic";

const DOMAIN = process.env.APP_URL || "https://freuly.de";
const SLUG = autoMobilitaetContent.slug;

export async function generateMetadata({ params }: { params: { lang: string } }) {
  const lang = isSupportedLang(params.lang) ? (params.lang as Lang) : "de";
  const c = autoMobilitaetContent.content[lang];
  return {
    title: c.metaTitle,
...
export default async function AutoMobilitaetPage({
  params,
}: {
  params: { lang: string };
}) {
  ...
  const { data } = await supabase
    ...
    .or(autoMobilitaetContent.filterOr ?? "")
    .limit(12);

  return (
    <CategoryContentPage
      lang={lang}
      content={autoMobilitaetContent.content[lang]}
      specialists={data ?? []}
    />
  );
}
