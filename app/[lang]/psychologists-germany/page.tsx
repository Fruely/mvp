...
import { psychologistsGermanyContent } from "@/content/seo/v2/psychologists-germany.content";

export const dynamic = "force-dynamic";

const DOMAIN = process.env.APP_URL || "https://freuly.de";
const SLUG = psychologistsGermanyContent.slug;

export async function generateMetadata({ params }: { params: { lang: string } }) {
  const lang = isSupportedLang(params.lang) ? (params.lang as Lang) : "de";
  const c = psychologistsGermanyContent.content[lang];
  return {
    title: c.metaTitle,
...
export default async function PsychologistsGermanyPage({
  ...
  const { data } = await supabase
    ...
    .or(psychologistsGermanyContent.filterOr ?? "")
    .limit(12);

  return (
    <CategoryContentPage
      lang={lang}
      content={psychologistsGermanyContent.content[lang]}
      specialists={data ?? []}
    />
  );
}
