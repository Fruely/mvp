...
import { tourenAusfluegeContent } from "@/content/seo/v2/touren-ausfluege.content";

export const dynamic = "force-dynamic";

const DOMAIN = process.env.APP_URL || "https://freuly.de";
const SLUG = tourenAusfluegeContent.slug;

export async function generateMetadata({ params }: { params: { lang: string } }) {
  const lang = isSupportedLang(params.lang) ? (params.lang as Lang) : "de";
  const c = tourenAusfluegeContent.content[lang];
  return {
    title: c.metaTitle,
...
export default async function TourenAusfluegePage({
  ...
  const { data } = await supabase
    ...
    .or(tourenAusfluegeContent.filterOr ?? "")
    .limit(12);

  return (
    <CategoryContentPage
      lang={lang}
      content={tourenAusfluegeContent.content[lang]}
      specialists={data ?? []}
    />
  );
}
