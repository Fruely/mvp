...
import { autokaufBeratungContent } from "@/content/seo/v2/autokauf-beratung.content";

export const dynamic = "force-dynamic";

const DOMAIN = process.env.APP_URL || "https://freuly.de";
const SLUG = autokaufBeratungContent.slug;

export async function generateMetadata({ params }: { params: { lang: string } }) {
  const lang = isSupportedLang(params.lang) ? (params.lang as Lang) : "de";
  const c = autokaufBeratungContent.content[lang];
  return {
    title: c.metaTitle,
...
export default async function AutokaufBeratungPage({
  params,
}: {
  params: { lang: string };
}) {
  ...
  const { data } = await supabase
    ...
    .or(autokaufBeratungContent.filterOr ?? "")
    .limit(12);

  return (
    <CategoryContentPage
      lang={lang}
      content={autokaufBeratungContent.content[lang]}
      specialists={data ?? []}
    />
  );
}
