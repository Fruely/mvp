...
const tableId = process.env.BASEROW_OPERATOR_CATEGORY_OPPORTUNITIES_TABLE_ID;
...
requireEnv('BASEROW_OPERATOR_CATEGORY_OPPORTUNITIES_TABLE_ID', tableId);
...
function mapRow(row) {
  const segment = [
    row['Подкатегория'] || row['Категория'],
    row['Город'] || row['Регион'] || row['Страна'],
  ].filter(Boolean).join(' / ');
  ...
  return {
    ...
    'Рекомендуемый канал действия': row['Рекомендуемый канал действия'] || '',
    'Операторский приоритет': row['Операторский приоритет'] || '',
    ...
    'Сигналы спроса': row['Сигналы спроса'] ?? null,
    'Сигналы предложения': row['Сигналы предложения'] ?? null,
    'Оценка перспективности': row['Оценка перспективности'] ?? null,
  };
}

async function main() {
  console.log('Reading Supabase operator category opportunities...');

  const supabaseRows = await supabaseGet(
    'growth_category_opportunities_operator_view?select=*&order=Оценка перспективности.desc&limit=200'
  );

  console.log(`Supabase rows: ${supabaseRows.length}`);

  console.log('Reading Baserow operator category opportunities...');
