...
requireEnv('BASEROW_CONTENT_TASKS_TABLE_ID', tableId);
...
async function main() {
  console.log('Reading Supabase content tasks...');

  const supabaseRows = await supabaseGet(
    'growth_content_tasks_view?select=*&order=Создано.desc&limit=50'
  );

  console.log(`Supabase rows: ${supabaseRows.length}`);

  console.log('Reading Baserow content tasks...');
