...
const tableId = process.env.BASEROW_OPERATOR_SCOUT_PROSPECTS_TABLE_ID;
...
requireEnv('BASEROW_OPERATOR_SCOUT_PROSPECTS_TABLE_ID', tableId);
...
function mapRow(row) {
  return {
    Name: row['Name'] || row['Описание услуги'] || row.id,
    'Supabase ID': row.id,
    ...
    'Рекомендуемый канал контакта': row['Рекомендуемый канал контакта'] || '',
    'Риск контакта': row['Риск контакта RU'] || row['Риск контакта'] || '',
    'Статус': row['Статус'] || '',
    'Статус контакта': row['Статус контакта'] || '',
    ...
    'AI-описание': row['AI-описание'] || '',
    'AI-оценка': row['AI-оценка'] ?? null,
    'Источник': row['Источник'] || '',
    'Доступные каналы': JSON.stringify(row['Доступные каналы'] || []),
  };
}

async function main() {
  console.log('Reading Supabase operator scout prospects...');

  const supabaseRows = await supabaseGet(
    'growth_scout_prospects_operator_view?select=*&order=Создано.desc&limit=200'
  );

  console.log(`Supabase rows: ${supabaseRows.length}`);

  console.log('Reading Baserow operator scout prospects...');
