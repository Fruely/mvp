...
const tableId = process.env.BASEROW_SCOUT_PROSPECTS_TABLE_ID;
...
requireEnv('BASEROW_SCOUT_PROSPECTS_TABLE_ID', tableId);
...
function mapRow(row) {
  ...
  return {
    ...
    'Риск контакта': row['Риск контакта'] || '',
...
async function main() {
  console.log('Reading Supabase scout prospects...');

  const supabaseRows = await supabaseGet(
    'growth_scout_prospects_view?select=*&order=Создано.desc&limit=50'
  );

  console.log(`Supabase rows: ${supabaseRows.length}`);

  console.log('Reading Baserow scout prospects...');
