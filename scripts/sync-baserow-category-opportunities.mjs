...
const tableId = process.env.BASEROW_CATEGORY_OPPORTUNITIES_TABLE_ID;
...
requireEnv('BASEROW_CATEGORY_OPPORTUNITIES_TABLE_ID', tableId);
...
function mapRow(row) {
  ...
  return {
    ...
    'Рекомендованное действие': row['Рекомендованное действие'] || '',
    'AI-вывод': row['AI-вывод'] || '',
  };
}

async function main() {
  console.log('Reading Supabase category opportunities...');

  const supabaseRows = await supabaseGet(
    'growth_category_opportunities_view?select=*&order=Создано.desc&limit=50'
  );

  console.log(`Supabase rows: ${supabaseRows.length}`);

  console.log('Reading Baserow category opportunities...');
  ...
  for (const supabaseRow of supabaseRows) {
    ...
    if (existing) {
      ...
      updated += 1;
      console.log(`Updated: ${supabaseRow.id}`);
    } else {
      await createBaserowRow(mapped);
      created += 1;
      console.log(`Created: ${supabaseRow.id}`);
    }
  }
