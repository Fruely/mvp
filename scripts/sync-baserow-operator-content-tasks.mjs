...
const tableId = process.env.BASEROW_OPERATOR_CONTENT_TASKS_TABLE_ID;
...
requireEnv('BASEROW_OPERATOR_CONTENT_TASKS_TABLE_ID', tableId);
...
function mapRow(row) {
  return {
    ...
    'Тема': row['Тема'] || '',
    'Готовый текст': row['Готовый текст'] || '',
    'CTA': row['CTA'] || '',
    'Целевая аудитория': row['Целевая аудитория'] || '',
    'Источник инсайта': row['Источник инсайта'] || '',
  };
}

async function main() {
  console.log('Reading Supabase operator content tasks...');

  const supabaseRows = await supabaseGet(
    'growth_content_tasks_operator_view?select=*&order=Приоритет.desc,Создано.desc&limit=200'
  );

  console.log(`Supabase rows: ${supabaseRows.length}`);

  console.log('Reading Baserow operator content tasks...');
