...
test("profile resolver supports RU, UA-to-UK, and DE parity", async () => {
  const tableRows = {
    specialist_profiles: [
      { specialist_id: "profile", about_me: "Legacy profile" },
    ],
    specialist_profile_translations: [
      { specialist_id: "profile", language_code: "ru", about_me: "Русский" },
      { specialist_id: "profile", language_code: "uk", about_me: "Українська" },
      { specialist_id: "profile", language_code: "de", about_me: "Deutsch" },
    ],
  };

  for (const [inputLocale, expected] of [
    ["ru", "Русский"],
    ["ua", "Українська"],
    ["de", "Deutsch"],
  ]) {
    const locale = toContentLocale(inputLocale);
    assert.ok(locale);
    const { client } = createMockClient(tableRows);
    const result = await resolveProfileContent(client, {
      specialistIds: ["profile"],
      locale,
    });
    assert.equal(result.get("profile")?.aboutMe, expected);
  }
});
