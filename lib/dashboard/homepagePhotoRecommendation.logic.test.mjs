...
test("CTA targets the existing dashboard homepage-photo editor", () => {
  ...
  assert.match(overview, /dashboardHomepagePhotoEditorHref\(lang\)/);
  assert.match(overview, /<HomepagePhotoRecommendationBanner/);
  assert.match(editor, /id=\{HOMEPAGE_PHOTO_EDITOR_ELEMENT_ID\}/);
  assert.match(editor, /<HomepagePhotoCropEditor/);
  assert.match(banner, /dashboard\.home\.homepagePhoto\.recommendCta/);
  assert.match(banner, /dashboard\.homepagePhoto\.title/);
});

test("dashboard banner reuses resolveHomepagePhotoState and does not parse metadata itself", () => {
  ...
  assert.match(overview, /shouldShowHomepagePhotoRecommendation\(/);
  assert.match(
    overview,
    /photo_url, photo_source_url, homepage_photo_url, homepage_photo, about_me/,
  );
});
