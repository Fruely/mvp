...
test("ready review sends complete profiles to activate, not a paid-gated publish fetch", () => {
  const review = source(
    "components/dashboard/onboarding/OnboardingReviewStep.tsx",
  );
  const finishSetup = review.indexOf("demandCopy.onboarding.finishSetup");
  const hardRequirements = review.indexOf(
    "dashboard.onboarding.reviewStep.hardRequirementsTitle",
  );

  assert.ok(finishSetup >= 0);
  assert.ok(hardRequirements >= 0);
  assert.ok(finishSetup < hardRequirements);
  assert.match(review, /specialist\/dashboard\/activate/);
  assert.doesNotMatch(review, /fetch\("\/api\/specialist\/dashboard\/publish"/);
