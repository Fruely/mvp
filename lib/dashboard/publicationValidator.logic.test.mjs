...
} = await import("./publicationValidator.ts");
const { getFirstIncompleteOnboardingStep } = await import("./onboardingStep.ts");
const { isPublicationReadyForDashboard } = await import("./publicationReadiness.ts");
const { isWithinDualRadius } = await import("../specialists/geography.ts");
...
test("hasValidServiceForPublish requires a positive price or a typed exception", () => {
...
test("onboarding resumes at basic when any required basic field is incomplete", () => {
  const validation = validatePublication({ ...BONN, city: "" });
  assert.equal(getFirstIncompleteOnboardingStep(validation), "basic");
});

test("onboarding resumes at services when basic is complete and service is missing", () => {
  const validation = validatePublication({
    ...BONN,
    servicesInSelectedCategory: [],
  });
  assert.equal(getFirstIncompleteOnboardingStep(validation), "services");
});

test("onboarding resumes at review when required fields are complete", () => {
  const validation = validatePublication({
    ...BONN,
    hasAbout: false,
    hasPhoto: false,
  ...
  assert.equal(validation.recommendations.length, 3);
  assert.equal(getFirstIncompleteOnboardingStep(validation), "review");
});

test("missing homepage crop does not block publication or change recommendations", () => {
  const r = validatePublication({
    ...BONN,
    hasAbout: true,
    hasPhoto: true,
  ...
  assert.ok(!r.blocking.some((issue) => String(issue.code).includes("homepage")));
  assert.ok(!r.recommendations.some((item) => String(item.code).includes("homepage")));
});
