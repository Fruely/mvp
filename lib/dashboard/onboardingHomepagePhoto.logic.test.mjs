...
test("photo step mounts shared HomepagePhotoCropEditor beside MAIN avatar uploader", () => {
  ...
  assert.match(photoStep, /import HomepagePhotoCropEditor from "@\/components\/specialist\/HomepagePhotoCropEditor"/);
  assert.match(photoStep, /<HomepagePhotoCropEditor/);
  ...
  assert.match(photoStep, /dashboard\.onboarding\.homepagePhoto\.optionalHint/);
  assert.match(wizard, /<OnboardingPhotoStep/);
  assert.equal((photoStep.match(/<HomepagePhotoCropEditor/g) || []).length, 1);
});

test("MAIN uploader and homepage editor stay separate concerns", () => {
  const photoStep = source("components/dashboard/onboarding/OnboardingPhotoStep.tsx");
  const mainFormStart = photoStep.indexOf("id={mainPhotoFormId}");
  const editorStart = photoStep.indexOf("<HomepagePhotoCropEditor");
  const navStart = photoStep.indexOf("dashboard.onboarding.nav.back");

  assert.ok(mainFormStart >= 0);
  assert.ok(editorStart > mainFormStart);
  assert.ok(navStart > editorStart);
...
test("onboarding loader reads homepage fields and passes them into the shared editor", () => {
  const page = source("app/[lang]/specialist/(protected)/dashboard/onboarding/page.tsx");
  const photoStep = source("components/dashboard/onboarding/OnboardingPhotoStep.tsx");
  const wizard = source("components/dashboard/onboarding/SpecialistOnboardingWizard.tsx");

  assert.match(page, /photo_source_url, homepage_photo_url, homepage_photo/);
  assert.match(page, /resolveHomepagePhotoState/);
  assert.match(page, /specialistId=\{specialist\.id\}/);
  assert.match(page, /homepagePhoto=\{homepagePhoto\}/);
  assert.match(wizard, /initialSourceUrl=\{homepagePhoto\.photo_source_url\}|homepagePhoto=\{homepagePhoto\}/);
  assert.match(photoStep, /initialSourceUrl=\{homepagePhoto\.photo_source_url\}/);
  assert.match(photoStep, /initialHomepagePhotoUrl=\{homepagePhoto\.homepage_photo_url\}/);
  assert.match(photoStep, /initialMetadata=\{homepagePhoto\.homepage_photo\}/);
  assert.match(photoStep, /canonicalOrigin=\{process\.env\.NEXT_PUBLIC_SUPABASE_URL \?\? null\}/);
});
...
test("review recommends homepage photo without making it a hard requirement", () => {
  ...
  assert.match(review, /dashboard\.onboarding\.reviewStep\.recommendHomepagePhoto/);
  assert.match(recommendationsBlock, /key: "homepagePhoto"/);
  assert.match(recommendationsBlock, /done: summary\.hasHomepagePhoto/);
  assert.match(recommendationsBlock, /neutralPending: true/);
  assert.doesNotMatch(hardItemsBlock, /homepagePhoto/);
  assert.match(page, /key: "homepagePhoto"/);
