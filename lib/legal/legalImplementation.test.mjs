...
test("partner agreement loads v1.3 from review source", () => {
  assert.equal(PARTNER_AGREEMENT_VERSION, "1.3");
  const de = getPartnerAgreement("de");
  assert.equal(de.version, "1.3");
  ...
  assert.match(plain, /gemeinsamen Haushalt/i);
  assert.match(plain, /Referral-Attribution/i);
});
