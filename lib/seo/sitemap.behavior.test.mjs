...
function loadSitemap(rows) {
  ...
  const dependencies = {
    ...
    "@/lib/specialists/publicCategoryCounts": {
      getPublicSpecialistCountsByServiceCategory: async (_client, categoryIds) =>
        new Map(categoryIds.map((id) => [id, id === "cat-psych" ? 1 : 0])),
    },
...
test("sitemap emits clean multilingual URLs and excludes empty categories", async () => {
  const { run } = loadSitemap({
    content_posts: [{ lang: "ru", slug: "guide", updated_at: "2026-09-01T00:00:00Z" }, { lang: "xx", slug: "hidden" }],
    categories: [
      { id: "cat-psych", slug: "psychologists" },
      { id: "cat-empty", slug: "barbers" },
      { id: "cat-uncat", slug: "uncategorized" },
      { id: "cat-cyr", slug: "психологи" },
    ],
  ...
  assert.ok(!urls.some((url) => url.includes("/specialists/barbers")));
