...
test("agreement version is v1.3 and stays synced with agreementMeta", () => {
  assert.equal(PARTNER_AGREEMENT_VERSION, "1.3");
  const meta = readFileSync(
    new URL("../../content/partners/agreementMeta.ts", import.meta.url),
    "utf8"
  );
  assert.match(meta, /PARTNER_AGREEMENT_VERSION = "1\.3"/);
  assert.match(meta, /PARTNER_AGREEMENT_PREVIOUS_VERSION = "1\.2"/);
  assert.match(meta, /PARTNER_AGREEMENT_INTERMEDIATE_VERSION = "1\.1"/);
  assert.match(meta, /PARTNER_REWARD_VALIDATION_DAYS = 14/);
  assert.match(meta, /PARTNER_AGREEMENT_EFFECTIVE_DATE = "2026-08-16"/);
...
test("German agreement source loads review partnerprogramm", () => {
  const src = readFileSync(
    new URL("../../content/partners/agreementContent.ts", import.meta.url),
    "utf8"
  );
  const review = readFileSync(
    new URL("../../docs/legal/final-review/partnerprogramm.de.md", import.meta.url),
    "utf8"
  );
  assert.match(src, /readReviewMarkdown\("partnerprogramm"/);
  assert.match(review, /Partnerprogramm-Bedingungen Freuly/);
  assert.match(review, /Version 1\.3/);
  assert.match(review, /§ 1 Anbieter und Geltungsbereich/);
  assert.match(review, /manuelle SEPA/);
  assert.match(review, /gemeinsamen Haushalt/);
  assert.match(review, /Referral-Attribution/);
  assert.doesNotMatch(review, /partner_attributions|payout_id/i);
  const meta = readFileSync(
    new URL("../../content/partners/agreementMeta.ts", import.meta.url),
    "utf8"
  );
...
test("RU/UA partner review sources include governing note and key rules", () => {
  const ru = readFileSync(
    new URL("../../docs/legal/final-review/partnerprogramm.ru.md", import.meta.url),
    "utf8"
  );
  const ua = readFileSync(
    new URL("../../docs/legal/final-review/partnerprogramm.ua.md", import.meta.url),
    "utf8"
  );
  assert.match(ru, /юридически определяющей является немецкая версия/i);
  assert.match(ua, /юридично визначальною є німецька версія/i);
  assert.match(ru, /14/);
  assert.match(ua, /14/);
  assert.match(ru, /домохозяйстве/i);
  assert.match(ua, /домогосподарстві/i);
});
...
test("canonical DE agreement hash helper derives from live DE plain text and preserves v1.2", () => {
  const hashSrc = readFileSync(new URL("./agreementHash.ts", import.meta.url), "utf8");
  assert.match(hashSrc, /getGermanAgreementPlainText/);
  assert.match(hashSrc, /getGermanAgreementPlainTextV12/);
  assert.match(hashSrc, /sha256/);
  const review = readFileSync(
    new URL("../../docs/legal/final-review/partnerprogramm.de.md", import.meta.url),
    "utf8"
  );
  assert.match(review, /§ 7 Auszahlung per manueller SEPA/);
});
