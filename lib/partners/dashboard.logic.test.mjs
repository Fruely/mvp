...
test("computeDashboardAmounts splits pending/approved/paid", () => {
  ...
  assert.equal(totals.approved_unpaid_cents, 2900);
  assert.equal(totals.paid_cents, 2900);
  assert.equal(totals.total_earned_cents, 5800);
  assert.equal(totals.available_for_payout_cents, 2900);
});
