...
export type SpecialistProfileSession =
  | { kind: "unauthorized" }
  | { kind: "specialist_required" }
  | { kind: "forbidden_blocked" }
  | { kind: "ok"; userId: string; specialistId: string; specialistStatus: string | null };

async function lookupSpecialistForUser(userId: string): Promise<SpecialistProfileSession> {
  ...
  if (error) {
    console.error("[specialistProfile/session] specialist lookup failed", error.message);
...
/** Bearer-only session for Native profile read. */
export async function resolveSpecialistProfileBearerSession(
  request: Request,
): Promise<SpecialistProfileSession> {
...
/** Bearer first, cookie fallback — shared profile mutations with Web dashboard. */
export async function resolveSpecialistProfileSession(
  request: Request,
): Promise<SpecialistProfileSession> {
  const bearer = await resolveBearerAuthUser(request);
...
export function specialistProfileSessionErrorStatus(
  session: Exclude<SpecialistProfileSession, { kind: "ok" }>,
): number {
  switch (session.kind) {
    case "unauthorized":
      return 401;
...
export function specialistProfileSessionErrorCode(
  session: Exclude<SpecialistProfileSession, { kind: "ok" }>,
): string {
  switch (session.kind) {
    case "unauthorized":
