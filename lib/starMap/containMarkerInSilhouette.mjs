...
import { EUROPE_GERMANY_PATH } from "./europeGermanyPath.mjs";

/** Inset so a 6–8px core plus glow stays inside the stylized border. */
export const STAR_MAP_SILHOUETTE_INSET = 12;

/** Smaller inset for the larger Europe viewBox (850×680). */
export const EUROPE_STAR_MAP_INSET = 6;
...
export const EUROPE_GERMANY_POLYGON = parseSvgPathToPolygon(EUROPE_GERMANY_PATH);
...
const POLYGON_CENTROID_CACHE = new Map([[GERMANY_POLYGON, polygonCentroid(GERMANY_POLYGON)]]);

function getPolygonCentroid(polygon) {
  const cached = POLYGON_CENTROID_CACHE.get(polygon);
  if (cached) return cached;
  const centroid = polygonCentroid(polygon);
  POLYGON_CENTROID_CACHE.set(polygon, centroid);
  return centroid;
}
...
function insetTowardCentroid(start, margin, polygon = GERMANY_POLYGON) {
  ...
  const dx = centroid.x - start.x;
  const dy = centroid.y - start.y;
