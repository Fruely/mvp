...
test("parseManagedHomepageSourceIdentity accepts only own storage source paths", () => {
  assert.equal(parseManagedHomepageSourceIdentity(`storage:${SOURCE_PATH}`, SPECIALIST_ID), SOURCE_PATH);
  assert.equal(parseManagedHomepageSourceIdentity(` storage:${SOURCE_PATH} `, SPECIALIST_ID), SOURCE_PATH);
  assert.equal(parseManagedHomepageSourceIdentity(SOURCE_URL, SPECIALIST_ID), null);
  assert.equal(parseManagedHomepageSourceIdentity(`url:${SOURCE_URL}`, SPECIALIST_ID), null);
  assert.equal(parseManagedHomepageSourceIdentity(`storage:${HOMEPAGE_PATH}`, SPECIALIST_ID), null);
  assert.equal(parseManagedHomepageSourceIdentity(`storage:${SPECIALIST_ID}/gallery/a.jpg`, SPECIALIST_ID), null);
  assert.equal(parseManagedHomepageSourceIdentity(`storage:${MAIN_PATH}`, SPECIALIST_ID), null);
  assert.equal(parseManagedHomepageSourceIdentity(`storage:${OTHER_ID}/source/a.jpg`, SPECIALIST_ID), null);
  assert.equal(parseManagedHomepageSourceIdentity(`storage:${SPECIALIST_ID}/source/../a.jpg`, SPECIALIST_ID), null);
  assert.equal(parseManagedHomepageSourceIdentity("not-storage", SPECIALIST_ID), null);
});

test("isManagedHomepageOutputPath accepts specialist homepage jpg only", () => {
  assert.equal(isManagedHomepageOutputPath(HOMEPAGE_PATH, SPECIALIST_ID), true);
  assert.equal(isManagedHomepageOutputPath(`${SPECIALIST_ID}/homepage/a.JPG`, SPECIALIST_ID), true);
  assert.equal(isManagedHomepageOutputPath(`${SPECIALIST_ID}/homepage/a.png`, SPECIALIST_ID), false);
  assert.equal(isManagedHomepageOutputPath(SOURCE_PATH, SPECIALIST_ID), false);
  assert.equal(
    homepageOutputIdentityFromPath(HOMEPAGE_PATH, SPECIALIST_ID),
    photoIdentityFromStoragePath(HOMEPAGE_PATH),
  );
});

test("isHomepagePhotoCropAspectValid allows 31:20 with 1px integer rounding", () => {
  assert.equal(isHomepagePhotoCropAspectValid(775, 500), true);
  assert.equal(isHomepagePhotoCropAspectValid(1550, 1000), true);
  assert.equal(isHomepagePhotoCropAspectValid(31, 20), true);
  assert.equal(isHomepagePhotoCropAspectValid(776, 500), true);
  assert.equal(isHomepagePhotoCropAspectValid(775, 501), true);
  assert.equal(isHomepagePhotoCropAspectValid(777, 500), false);
  assert.equal(isHomepagePhotoCropAspectValid(400, 400), false);
  assert.equal(isHomepagePhotoCropAspectValid(1550, 900), false);
});
