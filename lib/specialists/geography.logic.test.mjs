...
test("allowed radii include public 10/30/50/100 and legacy 5/25", () => {
  for (const km of ALLOWED_SERVICE_RADII_KM) {
    assert.equal(isAllowedServiceRadiusKm(km), true);
  }
  assert.ok(ALLOWED_SERVICE_RADII_KM.includes(30));
});

test("reject invalid radii", () => {
  for (const bad of [0, -1, 15, 200, 1000, NaN, Infinity, "25", null]) {
...
test("public location: online shows format plus city metadata", () => {
  const loc = getPublicSpecialistLocation({
    workFormat: "online",
    city: "Bonn",
    postalCode: "53115",
  ...
  assert.equal(loc.label, "Онлайн · Bonn");
  assert.equal(loc.kind, "online");
});

test("public location: online without city is format-only", () => {
  const loc = getPublicSpecialistLocation({
    workFormat: "online",
    onlineLabel: "Онлайн",
  });
