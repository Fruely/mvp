...
export type SpecialistServicesContext =
...
export async function resolveSpecialistServicesContext(
  request: Request,
): Promise<SpecialistServicesContext> {
  ...
  if (error) {
    console.error("[specialistServices/context] specialist lookup failed", error.message);
  ...
  return {
    ...
    categoryId: typeof specialist.category_id === "string" ? specialist.category_id : null,
