...
/** Enough rows for seeded shuffle + backfill; was 160 (over-fetched). */
const POOL_FETCH_LIMIT = 48;
...
function selectRecommendedRows(
  founderPool: SpecialistRow[],
  premiumPool: SpecialistRow[],
  discoveryPool: SpecialistRow[],
  now: Date
): SelectedSpecialist[] {
  ...
  let discoveryPick = takeUniqueById(
    seededShuffle(
      premiumPool.filter((s) => !used.has(s.id)),
      seedHalfDay ^ 0x50524d
    ),
    used,
  ...
  const toShow: SelectedSpecialist[] = [...founderPick, ...premiumPick, ...discoveryPick];
...
export async function fetchRecommendedSpecialists(
  ...
  const toShow = selectRecommendedRows(founderPool, premiumPool, discoveryPool, now);
  ...
  const [profilesResult, profileContentById, categoriesResult, ratingRowsResult] =
    await Promise.all([
      supabase
      ...
      contentLocale
        ? resolveProfileContent(supabase, {
            ...
            console.error("[homepage/recommended] profile localization failed", error);
            return null;
          })
  ...
  for (const category of categoriesResult.data ?? []) {
    categoryById.set(String(category.id), {
  ...
  for (const r of ratingRowsResult.data ?? []) {
  ...
  return toShow.map((row, index) => {
    ...
    return {
      ...
      about_line: profileContentById?.get(row.id)?.aboutMe ?? profile?.about_me ?? null,
