...
test("toHomepageRecommendedDto trims payload to card fields", () => {
  ...
  assert.equal(rows[0].photo_focus, null);
  assert.equal(rows[0].homepage_card_image_url, null);

  const withFocus = toHomepageRecommendedDto([
    {
      id: "1",
      slug: "one",
      name: "One",
      avatar_url: "https://example.com/a.jpg",
      homepage_card_image_url: "https://example.com/homepage.jpg",
      city: "Berlin",
      languages: ["ru"],
      category_title: "Cat",
      category_title_ru: "Cat",
      category_title_de: "Cat",
      category_title_ua: "Cat",
      photo_focus: { version: 1, status: "ok" },
    },
  ]);
  assert.deepEqual(withFocus[0].photo_focus, { version: 1, status: "ok" });
  assert.equal(withFocus[0].avatar_url, "https://example.com/a.jpg");
  assert.equal(withFocus[0].homepage_card_image_url, "https://example.com/homepage.jpg");
});
