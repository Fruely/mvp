...
test("K: pricing copy has free publication and no obsolete 7-day publication trial", () => {
  for (const blob of [ruLocale, uaLocale, deLocale]) {
    const data = JSON.parse(blob);
    const pricing = JSON.stringify(data.pricing);
    assert.match(pricing, /бесплат|безкоштов|kostenlos/i);
    assert.doesNotMatch(pricing, /7-днев|7-денн|7-täg|7 Kalendertage nach der ersten öffentlichen Veröffentlichung/i);
  }
});
...
test("O: FAQ contains no-results-guarantee disclaimer", () => {
  const ru = JSON.parse(ruLocale);
  const guaranteeFaq = ru.pricing.faq.find((item) => /гарантир/i.test(item.q));
  assert.ok(guaranteeFaq);
  assert.match(guaranteeFaq.a, /не гарант/i);
});
...
test("Q: DE/RU/UA pricing keys complete", () => {
  for (const blob of [ruLocale, uaLocale, deLocale]) {
    ...
    assert.ok(data.pricing.faq.length >= 6);
  }
});
