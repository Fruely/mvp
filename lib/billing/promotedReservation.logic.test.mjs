...
test("reservation checkout does not gate published promotions by source locale", () => {
  const checkoutSrc = readFileSync(
    new URL("./createPromotedReservationCheckout.ts", import.meta.url),
    "utf8",
  );
  assert.match(checkoutSrc, /getPublishedPromotionForCapture/);
  assert.doesNotMatch(checkoutSrc, /promotion\.locale !== input\.lang/);
});
