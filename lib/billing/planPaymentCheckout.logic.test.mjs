...
registerHooks({
  resolve(specifier, context, nextResolve) {
    const map = {
      ...
      react: new URL("./testMocks/react-cache.mjs", import.meta.url).href,
    };
...
test("client cannot supply amount/price/discount/currency", () => {
  for (const key of ["amount", "currency", "price_id", "discount", "promoted_credit_id", "success_url", "redirect_url"]) {
...
test("feature flag false: legacy path only, no plan_payments in route", () => {
  const manualIdx = createCheckoutSrc.indexOf("if (isManualRenewalEnabled())");
...
test("native return_target uses HTTPS bounce URLs on the Stripe session", async () => {
  resetBillingHarness();
  seedSpecialistPlan();
  ...
  assert.equal(
    session.success_url,
    "https://freuly.de/api/billing/checkout-return?checkout=success&plan=basic&target=native",
  );
  assert.equal(
    session.cancel_url,
    "https://freuly.de/api/billing/checkout-return?checkout=cancelled&plan=basic&target=native",
  );
  assert.doesNotMatch(session.success_url, /^freuly:/);
});
