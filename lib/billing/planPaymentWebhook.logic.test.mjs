...
const listingReconcileSqlSrc = readFileSync(
  new URL("../../supabase/manual_migrations/2026-09-15_reconcile_listing_independent_of_inactive.sql", import.meta.url),
  "utf8",
);
...
test("lifecycle: inactive subscription does not set billing_visibility_blocked", async () => {
  const oldExpires = isoDaysFromNow(-15);
  seedPaidPlanPayment({
    specialistPlan: { expires_at: oldExpires, plan_code: "basic" },
    planPayment: { period_end_at: oldExpires },
...
test("lifecycle: previously blocked listing is cleared when reconcile is inactive", async () => {
  const oldExpires = isoDaysFromNow(-15);
  seedPaidPlanPayment({
    specialistPlan: { expires_at: oldExpires, plan_code: "basic" },
    planPayment: { period_end_at: oldExpires },
...
test("lifecycle: production test case — stale basic/active refunded → grace", async () => {
  const refundedAt = isoDaysFromNow(-1);
  resetWebhookHarness();
  seedSubscriptionCustomer();
...
test("SQL migration uses GREATEST expression without pg_catalog schema qualification", () => {
  assert.doesNotMatch(reconcileSqlSrc, /pg_catalog\.greatest/i);
  assert.match(reconcileSqlSrc, /GREATEST\s*\(/);
});

test("listing reconcile migration does not hide specialists on inactive", () => {
  assert.match(listingReconcileSqlSrc, /CREATE OR REPLACE FUNCTION public\.reconcile_specialist_access/);
  assert.match(listingReconcileSqlSrc, /billing_visibility_blocked = false/);
  assert.doesNotMatch(listingReconcileSqlSrc, /SET billing_visibility_blocked = true/);
  assert.match(listingReconcileSqlSrc, /Listing eligibility is independent of subscription lifecycle/);
});
