...
export function resetBillingHarness() {
  ...
  billingHarness.planPaymentInsertError = null;
  billingHarness.planPaymentUpdateError = null;
  billingHarness.paymentUpdateError = null;
  billingHarness.stripeShouldFail = false;
  billingHarness.couponShouldFail = false;
  billingHarness.customerShouldFail = false;
  billingHarness.stripeExpireShouldFail = false;
  billingHarness.stripeSessions = [];
  billingHarness.stripeCoupons = [];
  billingHarness.stripeExpiredSessionIds = [];
  billingHarness.stripePriceOverrides = {};
...
const PLAN_PAYMENT_CREDIT_RESERVED_STATUSES = new Set([
  "pending",
  "checkout_created",
  "paid",
  "refunded",
...
function isPromotedCreditReserved(row) {
  return row.promoted_credit_id && PLAN_PAYMENT_CREDIT_RESERVED_STATUSES.has(row.status);
}

function defaultStripePrice(priceId) {
...
function tableRows(table) {
  ...
  if (table === "plan_payments") return billingHarness.planPayments;
...
class Query {
  ...
  handleInsert() {
    if (this.table === "plan_payments") {
      if (billingHarness.planPaymentInsertError) {
        return { data: null, error: billingHarness.planPaymentInsertError };
      }
      if (this.payload?.promoted_credit_id) {
        const conflict = billingHarness.planPayments.find(
          (row) =>
            row.promoted_credit_id === this.payload.promoted_credit_id &&
            isPromotedCreditReserved(row),
        );
        if (conflict) {
          return {
            data: null,
            error: { code: "23505", message: "uq_plan_payments_promoted_credit_reserved" },
          };
        }
      }
      const row = {
        id: `pp-${billingHarness.planPayments.length + 1}`,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
        ...this.payload,
      };
      billingHarness.planPayments.push(row);
  ...
  handleUpdate() {
    if (this.table === "plan_payments") {
      if (
        billingHarness.planPaymentUpdateError &&
        this.payload?.status === "checkout_created"
      ) {
        return { data: null, error: billingHarness.planPaymentUpdateError };
      }
      ...
      for (const row of matches) {
        ...
        updated.push(this.projectRow(row));
      }
  ...
  async single() {
    ...
    if (this.operation === "update") {
      const result = this.handleUpdate();
