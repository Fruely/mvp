      ...
      sessions: {
        create: async (params) => {
          ...
          if (params.mode === "subscription") {
            return {
              id: "cs_test_subscription_checkout",
              url: "https://checkout.stripe.com/c/pay/test-subscription",
            };
          }
          const purpose =
            params.metadata?.purpose ?? params.payment_intent_data?.metadata?.purpose;
          if (purpose === "promoted_request_access") {
            return {
          ...
          return {
            id: "cs_test_plan_payment",
            url: "https://checkout.stripe.com/c/pay/test-plan-payment",
          };
        },
        retrieve: async (id) => {
          if (webhookHarness.stripeRetrieveShouldFail) {
            throw new Error("stripe_session_retrieve_failed");
          }
          const stored = webhookHarness.checkoutSessionById.get(id);
