...
test("F: trialing basic is not paid entitlement", () => {
  const e = resolveSpecialistEntitlements({ plan_code: "basic", plan_status: "trialing" });
  assert.equal(e.effectivePaidPlan, null);
  assert.equal(e.galleryLimit, 5);
});

test("paid entitlement: basic + active", () => {
  const e = resolveSpecialistEntitlements({ plan_code: "basic", plan_status: "active" });
  assert.equal(e.effectivePaidPlan, "basic");
});

test("paid entitlement: premium + active", () => {
  const e = resolveSpecialistEntitlements({ plan_code: "premium", plan_status: "active" });
  assert.equal(e.effectivePaidPlan, "premium");
});

test("paid entitlement: basic + grace", () => {
  const e = resolveSpecialistEntitlements({ plan_code: "basic", plan_status: "grace" });
  assert.equal(e.effectivePaidPlan, "basic");
});

test("paid entitlement: premium + grace_period", () => {
  const e = resolveSpecialistEntitlements({ plan_code: "premium", plan_status: "grace_period" });
  assert.equal(e.effectivePaidPlan, "premium");
});

test("paid entitlement: basic + early_access excluded", () => {
  const e = resolveSpecialistEntitlements({ plan_code: "basic", plan_status: "early_access" });
  assert.equal(e.effectivePaidPlan, null);
  assert.equal(e.galleryLimit, 5);
});

test("paid entitlement: premium + early_access excluded", () => {
  const e = resolveSpecialistEntitlements({ plan_code: "premium", plan_status: "early_access" });
  assert.equal(e.effectivePaidPlan, null);
  assert.equal(e.galleryLimit, 5);
});

test("paid entitlement: premium + trialing excluded", () => {
  const e = resolveSpecialistEntitlements({ plan_code: "premium", plan_status: "trialing" });
  assert.equal(e.effectivePaidPlan, null);
});

test("paid entitlement: expired and cancelled excluded", () => {
  for (const status of ["expired", "cancelled"]) {
    assert.equal(
      resolveSpecialistEntitlements({ plan_code: "basic", plan_status: status }).effectivePaidPlan,
      null,
    );
    assert.equal(
      resolveSpecialistEntitlements({ plan_code: "premium", plan_status: status }).effectivePaidPlan,
      null,
    );
  }
});
