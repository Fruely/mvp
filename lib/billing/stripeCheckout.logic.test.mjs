...
test("A: authenticated specialist checkout creates Stripe session server-side", () => {
  assert.match(checkoutRoute, /getUser\(\)/);
  assert.match(checkoutRoute, /\.eq\("user_id", user\.id\)/);
...
test("B: unauthenticated request rejected", () => {
  assert.match(checkoutRoute, /error: "unauthorized"/);
  assert.match(checkoutRoute, /status: 401/);
});

test("C: user without owned specialist rejected", () => {
  assert.match(checkoutRoute, /specialist_not_found/);
  assert.match(checkoutRoute, /status: 403/);
});
...
test("K: session metadata contains trusted specialist_id and internal_plan", () => {
  ...
  assert.match(billingCustomersSrc, /internal_plan: input\.internalPlan/);
});

test("L: subscription metadata contains trusted specialist_id/internal_plan", () => {
  assert.match(stripeProviderSrc, /subscription_data:/);
  assert.match(stripeProviderSrc, /specialist_id: input\.specialistId/);
  assert.match(stripeProviderSrc, /internal_plan: planConfig\.internalPlan/);
});
...
test("Q: success/cancel URLs built server-side with locale", () => {
  ...
  assert.match(billingUrlsSrc, /specialist\/dashboard\/billing/);
