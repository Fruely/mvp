...
test("legacy accepted leads remain unlocked without timestamp", () => {
  assert.equal(isLeadContactUnlocked({ status: "accepted", contact_unlocked_at: null }), true);
});
...
test("unlock-contacts route exists", () => {
  ...
  assert.match(src, /contact_unlocked_at/);
  assert.match(src, /specialist_id/);
});
...
test("unlock-contacts sends client email only on first persisted unlock", () => {
  ...
  assert.match(src, /if \(didPersistFirstUnlock && resultRow\.client_email\)/);
  assert.match(src, /\.is\("contact_unlocked_at", null\)/);
  const earlyReturn = src.indexOf("if (isLeadContactUnlocked(normalized))");
