...
const {
  ...
  summarizeCampaignContext,
} = await import("./validation.ts");
const { campaignLinkToRequestHref } = await import("./resolve.ts");
...
test("generated URLs contain no PII patterns", () => {
  ...
  assert.doesNotMatch(href, /client_name|email|phone/i);
});

test("service request accepts optional campaign attribution id", () => {
  ...
  assert.ok(!("error" in result));
  assert.equal(result.client_campaign_link_id, "33333333-3333-4333-8333-333333333333");
});
...
test("non-campaign service requests still work without attribution", () => {
  ...
  assert.ok(!("error" in result));
  assert.equal(result.client_campaign_link_id, null);
});

test("invalid campaign attribution id rejected", () => {
  const result = validateServiceRequestCreate({
    client_name: "Anna",
    client_email: "anna@example.com",
    description: "Need help",
    ...
    service_timing_type: "asap",
    locale: "ru",
    client_campaign_link_id: "not-a-uuid",
    hp: "",
  });
  assert.ok("error" in result);
});
