...
test("wizard: 'coach' near-matches Coaches → coaches", () => {
  assert.equal(matchCategoryAsciiSlug("coach", CATEGORIES), "coaches");
});
...
test("wizard flow: ServiceSearchFlow stamps ui lang separately from service language", () => {
  const src = readFileSync(
    new URL("../../components/search-flow/ServiceSearchFlow.tsx", import.meta.url),
    "utf-8",
  );
  assert.match(src, /uiLang:/);
  assert.match(src, /uiLang,/);
  const pageSrc = readFileSync(
    new URL("../../app/[lang]/service-search/page.tsx", import.meta.url),
    "utf-8",
  );
  assert.match(pageSrc, /uiLang=\{lang\}/);
  assert.match(pageSrc, /defaultLanguage=\{lang\}/);
});

test("UA UI + Russian service language keeps ui=ua and lang=ru for coaches online", () => {
  const url = buildServiceSearchCategoryUrl({
...
test("UA UI + Ukrainian service language keeps ui=ua while lang normalizes to uk", () => {
  const url = buildServiceSearchCategoryUrl({
    categorySlug: "coaches",
    language: "ua",
    format: "online",
...
test("RU nearby coaches URL keeps category, place, radius, and ui=ru", () => {
  const url = buildServiceSearchCategoryUrl({
    categorySlug: "coaches",
    language: "ru",
    format: "nearby",
...
test("UA nearby coaches URL keeps ui=ua independent of service-language uk", () => {
  const url = buildServiceSearchCategoryUrl({
    categorySlug: "coaches",
    language: "ua",
    format: "nearby",
  ...
  assert.equal(p.get("lang"), "uk");
  assert.equal(p.get("category"), "coaches");
  assert.equal(p.get("place"), "Berlin");
});

test("direct canonical coach hubs stay on the UI locale path", () => {
  assert.equal(getCategoryUrl("ua", "coaches"), "/ua/specialists/coaches");
  assert.equal(getCategoryUrl("ru", "coaches"), "/ru/specialists/coaches");
  assert.equal(getCategoryUrl("de", "coaches"), "/de/specialists/coaches");
});

test("psychologists online still uses category URL, not free-text q", () => {
  const url = buildServiceSearchCategoryUrl({
    categorySlug: "psychologists",
    language: "ru",
    format: "online",
...
test("unrelated free-text stays on q= and never gets category=coaches", () => {
  const url = buildServiceSearchResultsUrl({
...
test("filtered coach search does not canonical-redirect to hub (no loop)", () => {
  assert.equal(
    categorySlugForCanonicalSearch({
      category: "coaches",
      q: null,
...
test("specialists page retries online for empty nearby category and does not loop", () => {
  const src = readFileSync(
    new URL("../../app/specialists/page.tsx", import.meta.url),
    "utf-8",
  );
  assert.match(src, /shouldRetryOnlineForEmptyCategorySearch/);
  assert.match(src, /mode: "online"/);
  assert.doesNotMatch(src, /redirect\(serviceSearchHref\(uiLang\)\);\s*redirect\(serviceSearchHref/);
});
