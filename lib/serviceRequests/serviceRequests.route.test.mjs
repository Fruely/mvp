...
test("AI referral stores normalized AI attribution metadata", async () => {
  const req = publicCreateRequest({
    ...validBody,
  ...
  const res = await createPost(req);
  assert.equal(res.status, 200);
  assert.equal(harness.rows[0].acquisition_source, "chatgpt");
  assert.equal(harness.rows[0].acquisition_channel, "ai");
  assert.equal(harness.rows[0].ai_provider, "chatgpt");
  assert.equal(harness.rows[0].ai_interaction_type, "ai_referral");
  assert.equal(harness.rows[0].acquisition_attribution_confidence, "high");
});

test("client UTM cannot claim authenticated ai_agent traffic", async () => {
  const req = publicCreateRequest({
    ...validBody,
    source_path: "/de/request",
    acquisition: {
      ...
      fbclid: null,
      referrer: null,
      landing_path: "/de/request?utm_source=gemini&utm_medium=ai_agent",
      captured_at: "2026-09-20T18:00:00.000Z",
    },
  });
  ...
  assert.equal(res.status, 200);
  assert.equal(harness.rows[0].acquisition_channel, "ai");
  assert.equal(harness.rows[0].ai_provider, "gemini");
  assert.equal(harness.rows[0].ai_interaction_type, "ai_referral");
  assert.equal(harness.rows[0].acquisition_attribution_confidence, "medium");
});
