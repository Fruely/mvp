...
test("A: CTA links to canonical /{lang}/become-specialist", () => {
  assert.match(pageSrc, /href=\{`\/\$\{lang\}\/become-specialist`\}/);
});

test("B-C: CTA URL contains no public_token or attribution_token", () => {
  const ctaBlock = pageSrc.match(/<Link[\s\S]*?become-specialist[\s\S]*?<\/Link>/)?.[0] ?? "";
  assert.ok(ctaBlock.length > 0);
  assert.doesNotMatch(ctaBlock, /public_token/);
  assert.doesNotMatch(ctaBlock, /attribution/);
});
...
test("CTA uses locale page lang, not query injection", () => {
  assert.match(pageSrc, /getPublishedPromotionForCapture/);
  assert.match(pageSrc, /promotion\.locale !== lang/);
  assert.doesNotMatch(pageSrc, /become-specialist\$\{/);
});
