...
test("public preview offers login/register for guests without replacing the accept CTA", () => {
  assert.match(publicPageSrc, /login\?next=/);
  assert.match(publicPageSrc, /become-specialist/);
  assert.match(publicPageSrc, /serviceRequestPromotion\.signupCta\.body/);
...
test("public preview page uses safe view and accept CTA only for promoted", () => {
  ...
  assert.match(publicPageSrc, /getPublishedPromotionPublicView\(params\.public_token, lang\)/);
  ...
  assert.doesNotMatch(publicPageSrc, /view\.locale !== lang/);
  assert.doesNotMatch(acceptPageSrc, /view\.locale !== lang/);
});
