...
test("capture accepts a published UA promotion on the DE landing locale", async () => {
  seedPublishedPromotion({ locale: "ua" });
  const res = await captureGet(makeCaptureRequest(`lang=de&public_token=${PROMOTION_TOKEN}`));
  assert.equal(res.status, 204);
  assert.equal(harness.attributionRows.length, 1);
  assert.equal(harness.attributionRows[0].landing_locale, "de");
});
