...
test("draft copies keep source locale and optional translations", () => {
  const result = validatePromotionDraftInput({
    locale: "ua",
    copies: {
      ua: { title: "Потрібен фотограф", summary: "Портфоліо в Köln" },
      de: { title: "Fotograf gesucht", summary: "Portfolio in Köln" },
    },
  });
  assert.ok(!("error" in result));
  assert.equal(result.locale, "ua");
  assert.equal(result.public_title, "Потрібен фотограф");
  assert.equal(result.localized_copy.de.title, "Fotograf gesucht");
});
...
test("O-Q: public read model excludes PII and raw description fields", () => {
  ...
  assert.match(publicDataSrc, /PROMOTION_CAPTURE_SELECT/);
...
test("admin promotion actions are server-only", () => {
  ...
  assert.match(src, /generatePromotionDraftAdmin/);
...
test("admin block auto-generates one editable source copy", () => {
  const src = readFileSync(
    new URL(
  ...
  assert.match(src, /generatePromotionDraftAction/);
  assert.match(src, /Сгенерировать заново/);
  assert.match(src, /Сохранить черновик/);
  assert.match(src, /Опубликовать/);
  assert.match(src, /Исходный язык заявки/);
  assert.doesNotMatch(src, /PROMOTION_LOCALES\.map/);
  assert.doesNotMatch(src, /copiesFromPromotion/);
});

test("AI promotion draft generation keeps PII out of the selected request fields", () => {
  const src = readFileSync(new URL("./promotionDraftGenerator.ts", import.meta.url), "utf8");
  const selectBlock =
    src.match(/const PROMOTION_DRAFT_REQUEST_SELECT = \[[\s\S]*?\]\.join\(", "\);/)?.[0] ?? "";

  assert.match(src, /assertAdminSession/);
  assert.ok(selectBlock.length > 0);
  assert.doesNotMatch(selectBlock, /client_name/);
  assert.doesNotMatch(selectBlock, /client_email/);
  assert.doesNotMatch(selectBlock, /client_phone/);
  assert.match(src, /\[contact removed\]/);
  assert.match(src, /\[phone removed\]/);
  assert.match(src, /Never include or infer a person's name/);
});

test("publishing regenerates every target translation and fails closed", () => {
  const src = readFileSync(new URL("./translatePublicCopy.ts", import.meta.url), "utf8");
  assert.match(src, /for \(const locale of PROMOTION_LOCALES\)/);
  assert.match(src, /throw new Error\("TRANSLATION_FAILED"\)/);
  assert.doesNotMatch(src, /fillMissingLocalizedCopies/);
});

test("saving edited promotion copy returns it to draft", () => {
  const src = readFileSync(new URL("./promotionAdminData.ts", import.meta.url), "utf8");
  assert.match(src, /status: "draft"/);
  assert.match(src, /published_at: null/);
});
