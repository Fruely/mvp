...
test("F: invalid urgency rejected", () => {
  const result = validateServiceRequestCreate({ ...validBase, urgency: "tomorrow" });
  assert.ok("error" in result);
  assert.equal(result.error, "invalid urgency");
});

test("G: specific_date without desired_date rejected", () => {
  const result = validateServiceRequestCreate({
    ...validBase,
  ...
  assert.ok("error" in result);
  assert.match(result.error, /desired_date/i);
});
