...
export function resetHarness() {
  ...
  harness.signupBindingInsertError = null;
  harness.promotionCaptureError = null;
  harness.accessTokenRows = [];
  harness.notifyCalls = [];
...
function tableRows(table) {
  ...
  if (table === "service_request_promotion_signup_bindings") return harness.signupBindingRows;
  if (table === "service_request_access_tokens") return harness.accessTokenRows;
  if (table === "inbox_items" || table === "notification_outbox" || table === "service_request_matches") return [];
  return harness.rows;
}
...
class Query {
  ...
  filtered() {
    return tableRows(this.table).filter((row) =>
      this.filters.every((f) => {
        if (f.type === "in") {
          return Array.isArray(f.values) && f.values.includes(row[f.column]);
        }
  ...
  async single() {
    ...
    if (this.operation === "insert") {
      ...
      if (this.table === "service_request_promotion_signup_bindings") {
        if (harness.signupBindingInsertError) {
          return { data: null, error: harness.signupBindingInsertError };
        }
        const row = {
          id: `bind-${harness.signupBindingRows.length + 1}`,
          created_at: new Date().toISOString(),
          ...this.payload,
        };
        for (const field of ["attribution_id", "specialist_id", "user_id"]) {
          if (harness.signupBindingRows.some((existing) => existing[field] === row[field])) {
            return {
              data: null,
              error: {
                code: "23505",
                message: `duplicate key value violates unique constraint on ${field}`,
              },
            };
          }
        }
        harness.signupBindingRows.push(row);
        return { data: null, error: null };
      }
      if (this.table === "service_request_access_tokens") {
        const row = { id: `token-${harness.accessTokenRows.length + 1}`, ...this.payload };
        harness.accessTokenRows.push(row);
        return { data: row, error: null };
      }
      if (this.table === "inbox_items" || this.table === "notification_outbox" || this.table === "service_request_matches") {
        return { data: null, error: null };
      }
      if (harness.insertError) return { data: null, error: harness.insertError };
      if (harness.rows.some((existing) => existing.public_id && existing.public_id === this.payload.public_id)) {
        return {
          data: null,
          error: { code: "23505", message: "duplicate key value violates unique constraint on public_id" },
        };
      }
      if (
        this.payload.client_idempotency_key &&
        harness.rows.some((existing) => existing.client_idempotency_key === this.payload.client_idempotency_key)
      ) {
        return {
          data: null,
          error: {
            code: "23505",
            message: "duplicate key value violates unique constraint on client_idempotency_key",
          },
        };
      }
      const row = {
