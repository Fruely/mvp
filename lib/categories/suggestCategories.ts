...
/**
 * Category suggestion used by `GET /api/categories/suggest` and, as a plain
 * internal service call, by server features that need a compatibility category.
 * There is no HTTP loopback between platform endpoints.
 *
 * Ranking tiers (higher = better). Term matches always beat title matches at the
 * same match shape. Within terms, lang === queryLang gets +TERM_LANG_MATCH_BONUS
 * over lang IS NULL (same tier).
 *
...
export const CATEGORY_SUGGEST_DEFAULT_LIMIT = 8;
export const CATEGORY_SUGGEST_MAX_LIMIT = 50;
...
/** Stage names double as the log discriminator used by the public route. */
export type CategorySuggestStage =
  | "v_searchable_categories top"
  | "category_search_terms"
  | "v_searchable_categories ids"
  | "v_searchable_categories full";

export type CategorySuggestResult =
  | { ok: true; data: SuggestedCategory[] }
  | { ok: false; stage: CategorySuggestStage; error: unknown };
...
export function parseCategorySuggestLimit(raw: string | null): number {
  const n = Number.parseInt(raw ?? "", 10);
  if (!Number.isFinite(n) || n < 1) return CATEGORY_SUGGEST_DEFAULT_LIMIT;
  return Math.min(CATEGORY_SUGGEST_MAX_LIMIT, n);
}
...
function mapCategoryRow(row: SearchableCategoryRow): SuggestedCategory {
  return {
    id: row.category_id,
    slug: row.slug ?? "",
    title: row.title ?? "",
...
export async function suggestCategories(
  ...
  params: { query: string | null; langCode: string | null; limit: number },
): Promise<CategorySuggestResult> {
  const safeQuery = typeof params.query === "string" ? sanitizeForIlike(params.query) : "";
  ...
  if (!safeQuery) {
    ...
    if (topError) {
      return { ok: false, stage: "v_searchable_categories top", error: topError };
    }

    return {
      ok: true,
      data: (topRows ?? []).map((row) => mapCategoryRow(row as SearchableCategoryRow)),
    };
  }
  ...
  if (termError) {
    return { ok: false, stage: "category_search_terms", error: termError };
  }
  if (titleError) {
    return { ok: false, stage: "v_searchable_categories ids", error: titleError };
  }
  ...
  if (fullError) {
    return { ok: false, stage: "v_searchable_categories full", error: fullError };
  }
  ...
  return { ok: true, data: scored.slice(0, limit).map(({ row }) => mapCategoryRow(row)) };
}
