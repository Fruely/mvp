...
const { coerceSpecialistsUiLang } = await import("../search/specialistsUiLang.ts");
...
/**
 * UI locale mapping is canonical in coerceSpecialistsUiLang.
 * Empty/unknown values still default to ru on the specialists page.
 */
function toUiLang(lang) {
  return coerceSpecialistsUiLang(lang) ?? "ru";
}
...
test("app/specialists/page.tsx resolves UI lang via resolveSpecialistsUiLang", () => {
  const src = readFileSync(
    new URL("../../app/specialists/page.tsx", import.meta.url),
    "utf-8",
  );
  assert.match(src, /resolveSpecialistsUiLang/);
  assert.match(src, /uiParam: sp\.ui/);
