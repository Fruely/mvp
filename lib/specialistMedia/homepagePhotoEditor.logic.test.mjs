...
test("dashboard hosts the editor beside MAIN photo without replacing avatar upload", () => {
  ...
  assert.match(dashboardSrc, /HomepagePhotoCropEditor/);
  ...
  assert.match(profileSrc, /photo_source_url, homepage_photo_url, homepage_photo/);
  assert.match(onboardingSrc, /HomepagePhotoCropEditor/);
  assert.match(onboardingSrc, /\/api\/specialist\/avatar\/upload/);
});

test("homepage and MAIN surfaces still ignore homepage_photo_url", () => {
  const cardSrc = readFileSync(
    new URL("../../components/home/variantC/VariantCSpecialistCard.tsx", import.meta.url),
    "utf8",
  );
  const recommendedSrc = readFileSync(
    new URL("../homepage/fetchRecommendedSpecialists.ts", import.meta.url),
    "utf8",
  );
  assert.doesNotMatch(cardSrc, /homepage_photo_url/);
  assert.doesNotMatch(recommendedSrc, /homepage_photo_url/);
});
